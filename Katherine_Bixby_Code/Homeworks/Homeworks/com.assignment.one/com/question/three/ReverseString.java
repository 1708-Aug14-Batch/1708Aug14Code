...
public class ReverseString {
	...
	public static String reverseString(String theString) {
		...
		String tempString; // for containing each String char value to put on or take off the stack
		for(int i=0;i<theString.length();i++) { // go through the whole string
			tempString = theString.substring(i, i+1); // put the character there onto the String stack
			strStack.push(tempString);
		}
		...
		for(int i=0;i<theString.length();i++) {	// go through the stack
			tempString = strStack.pop(); // take the top value off the stack (which was the last character of the String
